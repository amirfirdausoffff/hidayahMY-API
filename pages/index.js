...
const apiGroups = [
  ...
  {
    name: 'Khatam Quran',
    description: 'Track Quran reading progress with target date (requires Bearer token)',
    tag: 'khatam',
    endpoints: [
      {
        method: 'POST', path: '/api/khatam',
        summary: 'Start a new khatam',
        description: 'Creates a new khatam with a target completion date. Only one active khatam per user.',
        auth: true,
        body: { target_date: '2026-08-22' },
        response: { success: true, khatam: { id: 'uuid', start_date: '2026-05-22', target_date: '2026-08-22', status: 'active' } },
      },
      {
        method: 'GET', path: '/api/khatam',
        summary: 'Get active khatam progress',
        description: 'Returns the user\'s active khatam with computed progress stats, daily pace, and on-track indicator.',
        auth: true,
        response: { success: true, khatam: { id: 'uuid', start_date: '2026-05-22', target_date: '2026-08-22', completed_surahs: { '1': true, '2': true }, total_surahs: 114, completed_count: 2, progress_percent: 1.8, days_elapsed: 0, days_remaining: 92, daily_surahs_needed: 1.2, is_on_track: true, status: 'active' } },
      },
      {
        method: 'PUT', path: '/api/khatam',
        summary: 'Update or abandon khatam',
        description: 'Update target date or set status to abandoned.',
        auth: true,
        body: { target_date: '2026-09-01', status: 'abandoned' },
        response: { success: true, khatam: { id: 'uuid', start_date: '2026-05-22', target_date: '2026-09-01', status: 'active' } },
      },
      {
        method: 'POST', path: '/api/khatam/mark',
        summary: 'Mark surah as read/unread',
        description: 'Toggle a surah\'s completion status. Auto-completes the khatam when all 114 surahs are marked.',
        auth: true,
        body: { surah_number: 1, is_read: true },
        response: { success: true, surah_number: 1, is_read: true, completed_count: 1, total_surahs: 114, is_completed: false },
      },
      {
        method: 'GET', path: '/api/khatam/history',
        summary: 'Get past khatam history',
        description: 'Returns completed and abandoned khatams (max 20).',
        auth: true,
        response: { success: true, history: [{ id: 'uuid', start_date: '2026-01-01', target_date: '2026-03-01', completed_count: 114, progress_percent: 100, is_completed: true, status: 'completed', duration_days: 58 }] },
      },
      {
        method: 'GET', path: '/api/khatam/stats',
        summary: 'Khatam analytics (admin)',
        description: 'Returns aggregated khatam stats across all users. Admin only.',
        auth: true, admin: true,
        response: { success: true, stats: { total_khatams: 50, active_count: 30, completed_count: 15, abandoned_count: 5, completion_rate: 75, avg_completion_days: 45, on_track: 20, behind_schedule: 10, distribution: { '0-25': 5, '25-50': 10, '50-75': 8, '75-100': 7 } } },
      },
    ],
  },
  {
